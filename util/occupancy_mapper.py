    ...
    def create(self) -> None:
        ...
        for scene in ["1a", "1b", "1c", "2a", "2b", "2c", "4a", "4b", "4c", "5a", "5b", "5c"]:
            for layout in [0, 1, 2]:
                ...
                if scene not in scene_bounds:
                    scene_bounds[scene] = {"x_min": scene_env.x_min,
                    ...
                    for ix, iy in np.ndindex(occupancy_map.shape):
                        ...
                        for i, env in enumerate(scene_env.rooms):
                            ...
                            if env.is_inside(x, z):
                                rooms[ix, iy] = i
                                break
                    np.save(str(ROOM_MAPS_DIRECTORY.joinpath(str(scene)).resolve()), np.array(rooms))

                save_filename = f"{scene}_{layout}"
                # Add position markers and create an occupancy image.
                commands = []
                for ix, iy in np.ndindex(occupancy_map.shape):
                    # Ignore occupied or impossible positions.
                    if occupancy_map[ix][iy] == 1 or occupancy_map[ix][iy] == -1:
                        continue
                    x, z = OccupancyMapper.get_occupancy_position(scene_env=scene_env, ix=ix, iy=iy)
                    # Free positions will be colored blue.
                    if occupancy_map[ix][iy] == 0:
                        color = {"r": 0, "g": 0, "b": 1, "a": 1}
                    # Non-navigable positions will be colored red.
                    else:
                ...
                # Disable physics. We're going to use cube primitives for our markers and we don't want them to jostle.
